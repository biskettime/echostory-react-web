...
interface ProfileEditModalProps {
  ...
  currentInfo?: string;
  currentImage?: string;
  onSave?: (nickname: string, info: string, image?: string) => void;
}
...
export function ProfileEditModal({ 
  ...
  const [profileImage, setProfileImage] = useState<string | null>(currentImage || null);
  ...
  const handleSave = () => {
    if (nickname.trim()) {
      console.log('🔄 ProfileEditModal handleSave called:', { 
        nickname: nickname.trim(), 
        info: info.trim(), 
        profileImage: profileImage ? 'has image' : 'no image' 
      });
      onSave?.(nickname.trim(), info.trim(), profileImage || undefined);
      console.log('🔄 ProfileEditModal save completed - parent will handle navigation');
      // onClose()는 호출하지 않음 - ChatScreen에서 저장 후 자동으로 채팅 설정으로 이동
    } else {
      console.log('🔄 ProfileEditModal handleSave - nickname is empty');
    }
  };
  ...
  const handleCancel = () => {
    ...
    setInfo(currentInfo);
    setProfileImage(currentImage || null);
    onClose();
  };
